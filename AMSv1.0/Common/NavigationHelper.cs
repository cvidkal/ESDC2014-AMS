...
namespace AMSv1._0.Common
{
    /// <summary>
    /// NavigationManager 协助在页面间进行导航。 它提供一些命令，用于
    /// 前后导航以及注册用于前进和后退的标准鼠标和键盘
    /// 快捷方式。 此外，它集成 SuspensionManger
    /// 以在页面之间导航时处理进程生命期管理和状态管理
    ///。
    /// </summary>
    /// <example>
    /// 若要利用 NavigationManager，请执行下面两步或
    /// 以 BasicPage 或除 BlankPage 以外的任何页项开始。
    /// 
    /// 1) 在某处创建一个 NaivgationHelper 实例(如
